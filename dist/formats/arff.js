...
exports.toARFF = function (dataset, relationName, featureExtractor) {
  ...
  var featureLookupTable = new FeaturesUnit.FeatureLookupTable(); // Extract the input attributes (- features):

  dataset.forEach(function (datum) {
    datum.input = featureExtractor(datum.input, {});
    if (!_.isObject(datum.input)) throw new Error("Expected feature vector to be a hash, but found " + JSON.stringify(datum.input));
    featureLookupTable.addFeatures(datum.input);
  }); // Extract the target attributes (- classes):

  dataset.forEach(function (datum) {
    if (!_.isArray(datum.output)) datum.output = [datum.output];
...
exports.toARFFs = function (outputFolder, mapFileNameToDataset, featureExtractor) {
  ...
  for (var relationName in mapFileNameToDataset) {
    fs.writeFileSync(outputFolder + "/" + relationName + ".arff", toARFFLocal(mapFileNameToDataset[relationName], relationName, featureLookupTable));
  }
};
/**
 * convert a single dataset to Weka ARFF string.
