 ...
 */
var _ = require('underscore'); // Calculating the median of an array basically involves sorting the array and picking the middle number. 
...
exports.variance = function (list) {
  sum = _.reduce(list, function (memo, num) {
    return memo + num;
  }, 0);
  exp = sum / list.length;
  sum2 = _.reduce(list, function (memo, num) {
    return memo + num * num;
  }, 0);
...
exports.listembed = function (label) {
  ...
  if (_.isObject(label) && !_.isArray(label)) // if ('classes' in JSON.parse(label))
