...
PrecisionRecall.prototype = {
  ...
  addCasesLabels: function addCasesLabels(expectedClasses, actualClasses) {
    ...
    var allTrue = true;
    if (!(Object.keys(expectedClasses)[0] in this.confusion)) this.confusion[Object.keys(expectedClasses)[0]] = {};
    if (!(Object.keys(actualClasses)[0] in this.confusion[Object.keys(expectedClasses)[0]])) this.confusion[Object.keys(expectedClasses)[0]][Object.keys(actualClasses)[0]] = 0;
    this.confusion[Object.keys(expectedClasses)[0]][Object.keys(actualClasses)[0]] += 1;

    for (var actualClass in actualClasses) {
      if (!(actualClass in this.confusion)) this.confusion[actualClass] = {};
  ...
  addCasesHash: function addCasesHash(expectedClasses, actualClasses, logTruePositives) {
    ...
    if (explanations['FP'].length == 0) delete explanations['FP'];
    if (explanations['FN'].length == 0) delete explanations['FN'];
    return explanations;
  },
  ...
  addCasesHashSeq: function addCasesHashSeq(expectedClasses, actualClasses, logTruePositives) {
    ...
    _.each(expectedClasses, function (expected, key, list) {
      if (expected.length == 2 || expected.length == 3) ex.push(expected);
    }, this); // ac = actualClasses
    ...
    var ac = actualClasses; // filling interdependencies between labels 
    ...
    _.each(ac, function (actual, key, list) {
      if (actual.length > 3) {
        ...
        this.dep[label][label].push(str); // intersection, different intents but actual intersection

        var found = _.filter(ac, function (num) {
          return num[0] != actual[0] && this.intersection(num[1], actual[1]) == true;
        }, this);

        _.each(found, function (sublabel, key, list) {
          if (!(sublabel[0] in this.dep[label])) this.dep[label][sublabel[0]] = [];
    ...
    explanations_detail['TP'] = [];
    explanations_detail['FP'] = [];
    explanations_detail['FN'] = [];
    var allTrue = true;
    ...
    for (var expectedClassindex in ex) {
      ...
      _.each(ac, function (acc, key, list) {
        if (ex[expectedClassindex][0] == acc[0]) {
          if (ex[expectedClassindex][1].length == 0 || acc[1][0] == -1) found = true;else {
            if (this.intersection(ex[expectedClassindex][1], acc[1])) found = true;
          }
        }
      }, this);
    ...
    this.count++; // _.each(explanations, function(value, key, list){ 
    // explanations[key] = _.sortBy(explanations[key], function(num){ num });
    // explanations[key].sort()
    // }, this)
  ...
  retrieveLabels: function retrieveLabels() {
    ...
    arlabels = _.sortBy(arlabels, function (num) {
      return arlabels[0];
    });
  ...
  calculateStatsNoReturn: function calculateStatsNoReturn() {
    ...
    if (Object.keys(this.labels).length > 3) {
      ...
      _.each(['Precision', 'Recall', 'F1'], function (param, key, list) {
        macro_stats[param] = _.pluck(list_lab, param); // macro_stats[param] = _.filter(macro_stats[param], function(elem){ return (!_.isNaN(elem) && elem >=0)  })

        macro_stats[param] = _.reduce(macro_stats[param], function (memo, num) {
          if (!_.isNaN(num) && num >= 0) {
            return memo + num;
          } else {
            return memo;
          }
        }) / macro_stats[param].length;
      }, this);
    ...
    this.shortStatsString = sprintf("Accuracy=%d/%d=%1.0f%% HammingGain=1-%d/%d=%1.0f%% Precision=%1.0f%% Recall=%1.0f%% F1=%1.0f%% timePerSample=%1.0f[ms]", this.TRUE, this.count, this.Accuracy * 100, this.FN + this.FP, this.FN + this.TP, this.HammingGain * 100, this.Precision * 100, this.Recall * 100, this.F1 * 100, this.timePerSampleMillis);
  },
