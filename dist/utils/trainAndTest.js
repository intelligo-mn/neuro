...
module.exports.testLite = function (classifier, testSet, explain) {
  ...
  for (label in labeltree) {
    for (type in labeltree[label]) {
      ...
      labeltree[label][type]['LIST'] = _.sortBy(lis, function (num) {
...
module.exports.test = function (classifier, testSet, verbosity, microAverage, macroSum) {
  ...
  for (var i = 0; i < testSet.length; ++i) {
    ...
    var explanations = currentStats.addCases(expectedClasses, actualClasses, verbosity > 2); //currentStats.addCasesLabels(expectedClasses, actualClasses);  // EREL: Not clear what this should do
...
module.exports.compare = function (classifier1, classifier2, dataset, explain) {
  ...
  for (var i = 0; i < testSetOriginal.length; ++i) {
    expectedClasses = list.listembed(testSetOriginal[i].output); // expectedClasses2 = list.listembed(testSet2[i].output)
...
module.exports.trainAndTestLite = function (createNewClassifierFunction, trainSet1, testSet1, verbosity, microAverage, macroSum) {
  // TRAIN:
  ...
  if (verbosity > 0) console.log("end training on " + trainSet.length + " samples, " + (trainSet.allClasses ? trainSet.allClasses.length + ' classes, ' : '') + elapsedTime + " [ms]"); // TEST:

  return module.exports.testLite(classifier, testSet,
  /*explain=*/
  verbosity - 1);
};
...
module.exports.trainAndTest_async = function (classifierType, trainSet, testSet, callback) {
  ...
  classifier.trainBatchAsync(trainSet_copy, function (err, results) {
    ...
    console.vlog("trained"); // module.exports.test_hash(classifier, testSet, function(error, results){
...
module.exports.trainAndTest_hash = function (classifierType, trainSet, testSet, verbosity) {
  var startTime = new Date();
  var classifier = new classifierType();
  ...
  var trainEnd = new Date().getTime();
  console.log("classifier is trained"); // stat_hash = module.exports.test_hash(classifier, testSet1, verbosity, microAverage, macroSum, classifier_compare);
...
module.exports.trainAndTest = function (classifierType, trainSet, testSet, verbosity, microAverage, macroSum) {
  // TRAIN:
  var classifier = new classifierType();
  if (verbosity > 0) console.log("\nstart training on " + trainSet.length + " samples, " + (trainSet.allClasses ? trainSet.allClasses.length + ' classes' : ''));
  var startTime = new Date();
...
module.exports.trainAndCompare = function (createNewClassifier1Function, createNewClassifier2Function, trainSet, testSet, verbosity) {
  // TRAIN:
