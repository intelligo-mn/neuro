...
var MultiLabelPassiveAggressive = function (opts) {
  ...
  this.weights = {//DUMMY_CLASS:{}
  };
  this.weights_sum = {//DUMMY_CLASS:{}
  };
  this.seenFeatures = {};
...
MultiLabelPassiveAggressive.prototype = {
  ...
  predict: function (features, averaging, explain) {
    ...
    var scores = {};
    if (explain > 0) var explanations = []; //		for (var feature in features) {
    ...
    scoresVector.sort(function (a, b) {
      return b[1] - a[1];
  ...
  update: function (sample, classes) {
    ...
    var ranks = this.predict(sample,
    /*averaging=*/
    false); // pairs of [class,score] sorted by decreasing score
    ...
    if (loss > 0) {
      ...
      if (r_score < Number.MAX_VALUE) hash.addtimes(this.weights[r], tau, sample); // weights[r] += tau*sample

      if (s_score > -Number.MAX_VALUE) hash.addtimes(this.weights[s], -tau, sample); // weights[s] -= tau*sample
    } // this.weights_sum = (this.weights + this.weights_sum);


    for (category in this.weights) hash.add(this.weights_sum[category], this.weights[category]);
  ...
  trainBatch: function (dataset) {
    ...
    for (var i = 0; i < this.retrain_count; ++i) dataset.forEach(function (datum) {
  ...
  classify: function (features, explain, withScores) {
    ...
    /*remove_unknown_features=*/
    true);
    var scoresVector = this.predict(features,
    /*averaging=*/
    true, explain);
    return multilabelutils.mapScoresVectorToMultilabelResult(scoresVector, explain, withScores,
    /*threshold=*/
    0);
  },
   ...
   */
  editFeatureValues: function (features, remove_unknown_features) {//		console.log("before: "+util.inspect(features));
    //		if (!('bias' in features))
    //			features['bias'] = 1.0;
