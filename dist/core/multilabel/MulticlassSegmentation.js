...
MulticlassSegmentation.prototype = {
  ...
  cheapestSegmentSplitStrategy: function cheapestSegmentSplitStrategy(words, accumulatedClasses, explain, explanations) {
    ...
    if (!cheapestSentenceClassificationCost) throw new Error("cheapestSegmentClassificationCosts[" + words.length + "] is empty"); //console.log("cheapestSentenceClassificationCost");		console.dir(cheapestSentenceClassificationCost);
    ...
    for (var i = 0; i < cheapestClassificationPath.length - 1; ++i) {
      var segment = words.slice(cheapestClassificationPath[i], cheapestClassificationPath[i + 1]).join(" "); //console.log(segment+":  ");

      var segmentCategoryWithExplain = this.classifySegment(segment, explain); //console.dir(segmentCategoryWithExplain);
