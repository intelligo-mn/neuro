...
PartialClassification.prototype = {
  ...
  trainBatch: function (dataset) {
    ...
    _(num_of_classifiers).times(function (n) {
      ...
      }, this); // classifier = new this.multilabelClassifierType();


      this.classifier[n].trainBatch(data); // classifier.trainBatch(data)
  ...
  classify: function (sample, explain, continuous_output) {
    ...
    _.each(this.classifier, function (classif, key, list) {
      ...
      if (explain > 0) labels.push(value.classes);else labels.push(value);
    ...
    if (explain > 0) {
      ...
      if (_.keys(negative) != 0) explanation = {
        positive: positive,
        negative: negative
      };
    }
    ...
    } : labels; // console.log(JSON.stringify(explanation, null, 4))
    // return (explain>0?
    // 	{
    // 		classes: labels, 
    // 		scores: scores,
