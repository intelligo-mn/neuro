...
var Homer = function (opts) {
  ...
  this.splitLabel = opts.splitLabel || function (label) {
    return label.split(/@/);
  };

  this.joinLabel = opts.joinLabel || function (superlabel) {
    return superlabel.join("@");
  };
...
Homer.prototype = {
  ...
  trainOnline: function (sample, labels) {
    ...
    for (var i in labels) this.allClasses[labels[i]] = true;

    return this.trainOnlineRecursive(sample, labels.map(this.splitLabel), this.root);
  },
  ...
  trainOnlineRecursive: function (sample, splitLabels, treeNode) {
    ...
    for (var i in splitLabels) {
      ...
      if (splitLabel.length > 1) {
        if (!mapSuperlabelToRest[superlabel]) mapSuperlabelToRest[superlabel] = [];
  ...
  trainBatch: function (dataset) {
    dataset = dataset.map(function (datum) {
      ...
      for (var i in normalizedLabels) this.allClasses[normalizedLabels[i]] = true;

      return {
        input: datum.input,
        output: normalizedLabels.map(this.splitLabel)
      };
    }, this); // [ [ 'Offer', 'Leased Car', 'Without leased car' ], [ 'Offer', 'Working Hours', '9 hours' ] ]
  ...
  trainBatchRecursive: function (dataset, treeNode) {
    ...
    dataset.forEach(function (datum) {
      ...
      for (var i in splitLabels) {
        ...
        if (splitLabel.length > 1) {
          // if it have more than one label (superlabel)
          if (!mapSuperlabelToRest[superlabel]) mapSuperlabelToRest[superlabel] = [];
      ...
      */
      ...
      for (var superlabel in mapSuperlabelToRest) {
        if (!(superlabel in mapSuperlabelToRestDataset)) mapSuperlabelToRestDataset[superlabel] = [];
  ...
  classify: function (sample, explain) {
    var splitLabels = this.classifyRecursive(sample, explain, this.root); //console.log("splitLabels:"+JSON.stringify(splitLabels));
  ...
  classifyRecursive: function (sample, explain, treeNode, depth) {
    if (!depth) depth = 1; // classify the superlabel 
    ...
    for (var i in superlabels) {
      ...
      var splitLabel = [superlabel]; // classifier of [Offer] types / second level / classifies Offer's parameters
      ...
      if (branch) {
        ...
        for (var j in branchLabels) splitLabels.push(splitLabel.concat(branchLabels[j]));

        if (explain > 0) explanations = explanations.concat(branchLabelsWithExplain.explanation);
      } else {
  ...
  fromJSONRecursive: function (treeNodeJson) {
    ...
    for (var superlabel in treeNodeJson.mapSuperlabelToBranch) treeNode.mapSuperlabelToBranch[superlabel] = this.fromJSONRecursive(treeNodeJson.mapSuperlabelToBranch[superlabel]);
  ...
  setFeatureLookupTableRecursive: function (featureLookupTable, treeNode) {
    if (treeNode.superlabelClassifier && treeNode.superlabelClassifier.setFeatureLookupTable) treeNode.superlabelClassifier.setFeatureLookupTable(featureLookupTable);

    for (var superlabel in treeNode.mapSuperlabelToBranch) this.setFeatureLookupTableRecursive(featureLookupTable, treeNode.mapSuperlabelToBranch[superlabel]);
  },
  ...
  newMultilabelClassifier: function () {
    ...
    if (this.featureLookupTable && classifier.setFeatureLookupTable) classifier.setFeatureLookupTable(this.featureLookupTable);
    return classifier;
  }
  /*
   * UTILS
