...
var BinarySegmentation = function BinarySegmentation(opts) {
  ...
  switch (opts.segmentSplitStrategy) {
    case 'shortestSegment':
      this.segmentSplitStrategy = this.shortestSegmentSplitStrategy;
      break;

    case 'longestSegment':
      this.segmentSplitStrategy = this.longestSegmentSplitStrategy;
      break;

    case 'cheapestSegment':
      this.segmentSplitStrategy = this.cheapestSegmentSplitStrategy;
      break;

    default:
      this.segmentSplitStrategy = null;
  }
...
BinarySegmentation.prototype = {
  ...
  trainOnline: function trainOnline(sample, classes) {
    ...
    for (var negativeClass in this.mapClassnameToClassifier) {
      if (!classes[negativeClass]) this.mapClassnameToClassifier[negativeClass].trainOnline(sample, 0);
    }
  },
  ...
  bestClassOfSegment: function bestClassOfSegment(segment, explain) {
    ...
    if (classes.classes.length == 0) {
      ...
      // FEATURES
      return [classes.classes[0], classes.scores[classes.classes[0]], classes['explanation']['positive']]; // return [classes.classes[0], classes.scores[classes.classes[0]]];
    }
  },
  ...
  cheapestSegmentSplitStrategy: function cheapestSegmentSplitStrategy(words, accumulatedClasses, explain, explanations) {
    ...
    for (var i = 0; i < path.length - 1; ++i) {
      ...
      var segment = words.slice(path[i], path[i + 1]).join(" "); //HERE
      ...
      if (segmentClasses.length > 0) accumulatedClasses[segmentClasses[0]] = true; // explanations = []

      if (explain > 0) {
        if (segmentClasses.length > 0) explanations.push([segmentClasses[0], segment, [path[i], path[i + 1]], segmentClassesWithExplain['explanation']['positive']]);
      }
  ...
  longestSegmentSplitStrategy: function longestSegmentSplitStrategy(words, accumulatedClasses, explain, explanations) {
    ...
    for (var currentEnd = 1; currentEnd <= words.length; ++currentEnd) {
      ...
      var nextSegmentClasses = nextSegmentClassesWithExplain.classes ? nextSegmentClassesWithExplain.classes : nextSegmentClassesWithExplain; //console.log("\t"+JSON.stringify(nextSegment) +" -> "+nextSegmentClasses)
  ...
  classify: function classify(sentence, explain) {
    ...
    if (this.segmentSplitStrategy && words.length >= minWordsToSplit) {
      ...
      classification = this.bestClassOfSegment(sentence, explain); // classification = this.classifySegment(sentence, explain);
      //HERER
      ...
      return explain > 0 ? {
        ...
        // FEATURES
        explanation: [[classification[0], sentence, [0, sentence.length - 1], classification[2]]] // explanation: [[classification[0], sentence, [0,sentence.length-1]]]

      } : classification[0]; // return {classes: classification[0],
