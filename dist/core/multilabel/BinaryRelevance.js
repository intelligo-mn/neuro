...
BinaryRelevance.prototype = {
  ...
  trainOnline: function trainOnline(sample, labels) {
    ...
    for (var negativeLabel in this.mapClassnameToClassifier) {
      if (labels.indexOf(negativeLabel) < 0) this.mapClassnameToClassifier[negativeLabel].trainOnline(sample, 0);
    }
  },
  ...
  trainBatch: function trainBatch(dataset) {
    ...
    for (var d in dataset) {
      ...
      for (var negativeLabel in this.mapClassnameToClassifier) {
        ...
        if (labels.indexOf(negativeLabel) < 0) mapClassnameToDataset[negativeLabel].push({
          input: sample,
          output: 0
        });
      }
    ...
    for (var label in mapClassnameToDataset) {
      ...
      this.mapClassnameToClassifier[label].trainBatch(mapClassnameToDataset[label]);
    }
  },
  ...
  classify: function classify(sample, explain, withScores) {
    ...
    for (var label in this.mapClassnameToClassifier) {
      ...
      if (this.debug) console.dir("Classify for class=" + label); // fs.writeFileSync('/tmp/labels/'+label, JSON.stringify(classifier.getFeatures(), null, 4), 'utf8');
  ...
  classifyBatch: function classifyBatch(testSet) {
    ...
    _.each(testSet, function (value, key, list) {
      ...
      _.each(results, function (ar, label, list) {
        if (ar[key] != 0) testSet[key]['output'].push(label);
      }, this);
    }, this);
  ...
  makeSureClassifierExists: function makeSureClassifierExists(label) {
    if (!this.mapClassnameToClassifier[label]) {
      ...
      this.mapClassnameToClassifier[label] = new this.binaryClassifierType();
      if (this.featureLookupTable && this.mapClassnameToClassifier[label].setFeatureLookupTable) this.mapClassnameToClassifier[label].setFeatureLookupTable(this.featureLookupTable);
    }
  }
};
