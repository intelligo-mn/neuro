...
kNN.prototype = {
  ...
  classify: function classify(sample, explain) {
    ...
    }, this); // is canceled due to "okay is fine"
    ...
    var distances = _.map(trainset, function (value) {
      return {
        'input': value['input'],
        'output': value['output'],
        'distance': _.reduce(this.distanceFunctionList, function (memo, df) {
          return memo + df(sample, value['input']);
        }, 0),
        'score': _.reduce(this.distanceFunctionList, function (memo, df) {
          return memo + df(sample, value['input']);
        }, 0) // 'distance': dfmap[this.distanceFunction](sample, value['input']),
        // 'score'   : this.distanceWeightening(_.reduce(this.distanceFunctionList, function(memo, df){ return memo + df(sample, value['input']); }, 0))

      };
    }, this);

    var distances = _.sortBy(distances, function (num) {
      return num['distance'];
    }); // eliminate Infinite and null


    var distances = _.filter(distances, function (num) {
      return !isNaN(parseFloat(num['distance'])) && isFinite(num['distance']);
    });
    ...
    var real_k = distances.length - _.find(distances.reverse(), function (num) {
      return num['distance'] == margin;
    });

    var knn = distances.slice(0, real_k);

    var output = _.groupBy(knn, function (num) {
      return num['output'];
    });
    ...
    }, this); // _.each(knn, function(val, key, list){ 
    // console.log(val)
  ...
  translaterow: function translaterow(row) {
    ...
    _.each(row, function (value, key, list) {
      if (value != 0) output[this.featureLookupTable['featureIndexToFeatureName'][key]] = value;
    }, this);
