...
function () {
  function Bayesian(options) {
    ...
    switch (backend.type.toLowerCase()) {
      ...
      case "localstorage":
        this.backend = new (require("./backends/localStorage").LocalStorageBackend)(backend.options);
        break;
  ...
  _createClass(Bayesian, [{
    key: "trainOnline",
    value: function trainOnline(document, category) {
      this.incDocCounts([{
        input: document,
        output: category
      }]);
    ...
    key: "trainBatch",
    value: function trainBatch(data) {
    ...
    key: "classify",
    value: function classify(document, explain) {
    ...
    key: "getProbsSync",
    value: function getProbsSync(document) {
      ...
      if (this.normalizeOutputProbabilities) {
        var sum = _(probs).reduce(function (memo, num) {
          return memo + num;
        }, 0);
    ...
    key: "bestMatch",
    value: function bestMatch(pairs) {
    ...
    key: "getCats",
    value: function getCats(callback) {
    ...
    key: "wordProb",
    value: function wordProb(word, cat, cats, wordCounts) {
      ...
      }, 0); // get weighted average with assumed so prob won't be extreme on rare words


      var modifiedProbGivenCat = (this.weight * this.assumed + totalWordCount * probWordGivenCat) / (this.weight + totalWordCount); //console.log("word="+word+" cat="+cat+" probWordGivenCat="+probWordGivenCat+" totalWordCount="+totalWordCount+" modifiedProbGivenCat="+modifiedProbGivenCat)

      return modifiedProbGivenCat;
    }
  }, {
    key: "getCatProbs",
    value: function getCatProbs(cats, document, counts) {
      var _this = this;
      ...
      _(cats).forEach(function (catCount, cat) {
        var catPriorProb = (catCount || 0) / numDocs; // The probability to see a document is the product
        ...
        var docProb = _(Object.keys(document)).reduce(function (prob, word) {
          ...
          var probWordGivenCat = _this.wordProb(word, cat, cats, wordCounts);

          var probWordsGivenCat = Math.pow(probWordGivenCat, document[word]); //console.log("probWordGivenCat="+probWordGivenCat+" probWordsGivenCat="+probWordsGivenCat+" document[word]="+document[word])

          return prob * probWordsGivenCat;
        }, 1); //console.log("docProb="+docProb)
    ...
    key: "getWordCounts",
    value: function getWordCounts(words, cats, callback) {
    ...
    key: "incDocCounts",
    value: function incDocCounts(samples, callback) {
      // accumulate all the pending increments
      var wordIncs = {};
      var catIncs = {};
      samples.forEach(function (sample) {
        var cat = sample.output; //if (_.isObject(cat))
