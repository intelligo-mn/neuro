...
SvmLinear.prototype = {
  ...
  trainBatch: function trainBatch(dataset) {
    ...
    _.each(dataset, function (datum, key, list) {
      if (_.isArray(datum.output)) if (datum.output.length > 1) {
        console.log("Multi-label is not allowed");
        console.log(JSON.stringify(darum.output, null, 4));
        process.exit(0);
      }
    }, this); //  convert all arraay-like outputs to just values


    dataset = _.map(dataset, function (datum) {
      if (_.isArray(datum.output)) datum.output = datum.output[0];
      return datum;
    });
    this.allLabels = _(dataset).map(function (datum) {
      return datum.output;
    });
    this.allLabels = _.uniq(_.flatten(this.allLabels)); // dataset = _.map(dataset, function(datum){
    // 	datum.output = this.allLabels.indexOf(datum.output)
    // 	return datum });

    if (this.allLabels.length == 1) // a single label
      return; //console.log(util.inspect(dataset,{depth:1}));

    if (this.debug) console.log("trainBatch start");
    var learnFile = svmcommon.writeDatasetToFile(dataset, this.bias,
    /*binarize=*/
    false, this.model_file_prefix + "_" + this.timestamp, "SvmLinear", FIRST_FEATURE_NUMBER);
  ...
  classifyBatch: function classifyBatch(trainset) {
    ...
    var testFile = svmcommon.writeDatasetToFile(trainset, this.bias,
    /*binarize=*/
    false, "/tmp/test_" + this.timestamp, "SvmLinear", FIRST_FEATURE_NUMBER);
  ...
  classify: function classify(features, explain, continuous_output) {
    ...
    if (this.allLabels.length == 1) {
      // a single label
      var result = !continuous_output ? this.allLabels[0] : !this.multiclass ? 1.0 : [[this.allLabels[0], 1.0]];
      return explain > 0 ? {
        classes: result,
    ...
    for (var label in this.mapLabelToMapFeatureToWeight) {
      ...
      var scoreWithExplain = svmcommon.classifyWithModelMap(mapFeatureToWeight, this.bias, features, explain,
      /*continuous_output=*/
      true, this.featureLookupTable);
      ...
      if (scoreWithExplain.explanation && explain > 0) {
        labelAndScore.push(this.multiclass ? scoreWithExplain.explanation.join(" ") : scoreWithExplain.explanation);
      }
    ...
    var result = !continuous_output ? labels[0][0] : !this.multiclass ? labels[0][0] > 0 ? labels[0][1] : labels[1][1] : labels;
    return explain > 0 ? {
      classes: result,
...
var LIB_LINEAR_MODEL_PATTERN = new RegExp("[\\S\\s]*" + // skip the beginning of string
...
function modelStringToModelMap(modelString) {
  ...
  var weightsMatrix = matches[3]; // each line represents a feature; each column represents a label:
  ...
  for (var feature in weightsLines) {
    ...
    if (weights.length == 0) continue; // ignore empty lines
    //		if (isNaN(parseFloat(weights[weights.length-1])))
    //			weights.pop();

    if (weights.length == 1 && labels.length == 2) weights[1] = -weights[0];
    if (weights.length != labels.length) throw new Error("Model does not match SVM-Linear format: there are " + labels.length + " labels (" + labels + ") and " + weights.length + " weights (" + weights + ")");

    for (var iLabel in labels) {
      var label = labels[iLabel];
      var weight = parseFloat(weights[iLabel]);
      if (Math.abs(weight) >= MIN_WEIGHT) mapLabelToMapFeatureToWeight[label][feature] = weight;
    }
  }
