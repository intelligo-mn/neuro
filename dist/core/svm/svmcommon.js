...
module.exports.classifyWithModelMap = function (modelMap, bias, features, explain, continuous_output, featureLookupTable) {
  ...
  if (!continuous_output) result = result > 0 ? 1 : 0;
  ...
  if (explain > 0) {
    ...
    var explanations = _.filter(explanations, function (num) {
      return num.relevance != 0;
    ...
    if (!this.detailed_explanations) {
      ...
      explanations = _.sortBy(explanations, function (num) {
        return num[1];
      }).reverse();
    }
