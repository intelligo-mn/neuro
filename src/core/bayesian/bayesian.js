...
export class Bayesian {
  constructor(options) {
    ...
    this.normalizeOutputProbabilities =
      options.normalizeOutputProbabilities || false;
    this.calculateRelativeProbabilities =
      options.calculateRelativeProbabilities || false;
    ...
    switch (backend.type.toLowerCase()) {
      case "redis":
        ...
        throw new Error(
          "Redis backend support was dropped, in order to remove dependencies"
        );
        break;
      case "localstorage":
   ...
   */
  classify(document, explain) {
    if (!_.isObject(document)) {
      throw new Error(
        "document should be a feature-value hash, but it is " +
  ...
  wordProb(word, cat, cats, wordCounts) {
  ...
  getCatProbs(cats, document, counts) {
  ...
  getWordCounts(words, cats, callback) {
