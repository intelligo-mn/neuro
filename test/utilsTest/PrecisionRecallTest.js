...
import { PrecisionRecall } from "../../dist/utils";
...
describe("PrecisionRecall object", function() {
  it("empty sample", function() {
    ...
    keys(pr.labels).length.should.equal(0);
  });

  it("confusion", function() {
    var pr = new PrecisionRecall();
    var expl = pr.addCasesHash([1], [1, 3], true);
    isEqual(pr.confusion, {
      "1": {
        "1": 1,
        "3": 1
      }
  ...
  it("correctly calculates precision, recall, etc.", function() {
    ...
    isEqual(expl.TP, ["1", "3", "5"]).should.be.true;
    isEqual(expl.FP, ["7"]).should.be.true;
    isEqual(expl.FN, ["2", "4"]).should.be.true;
    ...
    isEqual(expl.TP, ["1"]).should.be.true;
    isEqual(expl.FP, ["2", "8"]).should.be.true;
    isEqual(expl.FN, ["3", "4", "5"]).should.be.true;
    ...
    pr.microF1.should.equal(
      (2 * pr.microPrecision * pr.microRecall) /
        (pr.microPrecision + pr.microRecall)
    );
  ...
  it("correctly calculates precision, recall, accuracy in hash format.", function() {
    ...
    isEqual(stats["TP"], ["1", "3", "5"]).should.equal(true);
    isEqual(stats["FP"], ["7"]).should.equal(true);
    isEqual(stats["FN"], ["2", "4"]).should.equal(true);
