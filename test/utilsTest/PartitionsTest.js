...
describe("partitions", function() {
  ...
  it.skip("partitions_hash_fold", function() {
    var dataset = {
      label1: [1, 3, 5, 7, 9, 11, 13],
      label2: [0, 2, 4, 6, 8, 10, 12]
    };
    var data = partitions.partitions_hash_fold(dataset, 2, 1);
    isEqual(data["test"], [7, 9, 11, 6, 8, 10]).should.be.true;
    var data = partitions.partitions_hash_fold(dataset, 3, 2);
    isEqual(data["test"], [9, 11, 8, 10]).should.be.true;
  });
  ...
  it("partition hash", function() {
    ...
    partitions.partitions_hash(dataset, 2, function(train, test, index) {
  ...
  it("creates 5 partitions, with a test-set of 2 in each", function() {
    ...
    partitions.partitions(dataset, 5, function(train, test, index) {
      ...
      _(test)
        .intersection(train)
        .should.have.lengthOf(0); // most important test - make sure there is no leak from train to test!
      numOfPartitions++;
    });
