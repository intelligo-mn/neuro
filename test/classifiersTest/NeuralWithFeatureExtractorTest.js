...
describe("classifier with a single feature extractor for words", function() {
  it("classifies sentences", function() {
    ...
    spamClassifier.trainBatch([
      {
        input: "cheap replica watch es",
        output: [1]
      },
      {
        input: "your watch is ready",
        output: [0]
      },
      {
        input: "I don't know if this works on windows",
        output: [0]
      },
      {
        input: "cheap windows !!!",
    ...
    spamClassifier
      .classify("I don't know if this is a replica of windows")
      .should.be.below(0.2); // low number (not spam)
    ...
    spamClassifier
      .classify("watch")
      .should.be.above(0.3)
      .and.below(0.7); // medium number (not sure if spam)
  });
});
...
describe("classifier with an array of feature extractors, for words and bigrams", function() {
  it("classifies sentences", function() {
    var spamClassifier = new EnhancedClassifier({
      classifierType: NeuralNetwork,
      featureExtractor: [NGramsOfWords(1), NGramsOfWords(2)]
    });
