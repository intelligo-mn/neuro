...
import { multilabel, Winnow } from "../../../dist/core";

var retrain_count = 10;
var BinaryRelevanceWinnow = multilabel.BinaryRelevance.bind(this, {
...
var testMultiLabelClassifier = function(classifier) {
  ...
  it("classifies 2-class samples", function() {
    ...
    classifier
      ...
      .sort()
      .should.eql(["B", '{"C":"c"}'].sort());
    classifier
      .classify({ I: 1, want: 1, cc: 1, and: 1, aa: 1 })
      .sort()
      .should.eql(["A", '{"C":"c"}'].sort());
  });

  it("classifies 3-class samples", function() {
    classifier
      .classify({ I: 1, want: 1, aa: 1, and: 1, bb: 1, "'": true, cc: 1 })
      .sort()
      .should.eql(["A", "B", '{"C":"c"}'].sort());
  });
