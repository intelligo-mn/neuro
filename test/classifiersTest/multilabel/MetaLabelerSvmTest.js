...
function test(name, SvmClassifier) {
  ...
  var BinaryRelevanceSvm = multilabel.BinaryRelevance.bind(this, {
    binaryClassifierType: SvmClassifier
  });

  var MetaLabelerSvm = EnhancedClassifier.bind(this, {
    classifierType: multilabel.MetaLabeler.bind(this, {
  ...
  describe(
    "Meta-Labeler with " + name + ", batch-trained on Single-class inputs,",
    function() {
      ...
      it("explains its decisions", function() {
        var ab = classifier.classify(
          { I: 1, want: 1, aa: 1, and: 1, bb: 1 },
          /*explain=*/ 3
        );
  ...
  describe(
    "Meta-Labeler with " + name + ", batch-trained on two-class inputs,",
    function() {
      ...
      it("classifies 2-class samples", function() {
        classifier
          .classify({ I: 1, want: 1, aa: 1, and: 1, bb: 1 })
          .sorted()
          .should.eql(["A", "B"]);
        classifier
          .classify({ I: 1, want: 1, bb: 1, and: 1, cc: 1 })
          .sorted()
          .should.eql(["B", "C"]);
        classifier
          .classify({ I: 1, want: 1, cc: 1, and: 1, dd: 1 })
          .sorted()
          .should.eql(["C", "D"]);
        classifier
          .classify({ I: 1, want: 1, dd: 1, and: 1, aa: 1 })
          .sorted()
          .should.eql(["A", "D"]);
      });
...
if (SvmLinear.isInstalled())
  test(
    ...
    SvmLinear.bind(this, {
      learn_args: "-c 20.0",
      model_file_prefix: __dirname + "/../../tempfiles/SvmLinear"
    })
  );
else
  console.warn(
